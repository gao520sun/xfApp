...
import {getTangAuthor} from '../Api/TangApi'

interface TAuthorPageContainerProps {}

const TAuthorPageContainer = (props: TAuthorPageContainerProps) => {
    ...
    const _onAuthorListPress = (item) => {
        navigation.navigate('TAuthorPoetryPageContainer',{...item})
    }
    const fetchData = async ({offset,successCallback,errorCallback}) => {
        const res = await getTangAuthor({limit:10, offset:offset})
        if(res.status === 1) {
...
TAuthorPageContainer.navigationOptions = ({navigation}) => ({
    title:'作者',
  })

export default TAuthorPageContainer;
